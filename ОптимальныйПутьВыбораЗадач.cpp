/* На странной олимпиаде поступают задачи в разный момент времени Si, участники должны определить будут ли решать поступившую задачу
   ...
   Участнику "А", как и всем остальным, известно: 1) В какие моменты времени поступают все задачи.. 2) Сколько времени будет отведено на ту или иную задачу..
   3) Сколько балов он получит за решение!
*/
...
/* 
   ФОРМАТЫ: 
            --- Ввод: Первая строка входного файла содержит одно целое число n (i < n < 100000) - количество задач на олимпиаде, последующие же n строк
            содержат описание задач, по три числа на каждой строке - Si, Ti, Ci (i <= Si, Ti; Ci <= 10(в степени 9)) - момемнт появления задачи в минутах,
            время отведённое на решение в минутах, сколько баллов получит участник "А" за решение этой задачи ПРИ ЭТОМ если оптимальных ответов будет несколько, необходимо вывести любой из них
            --- Вывод: Первая строка выходного файла будет содержать одно число - максимальное колличество балов которые можно получить
            Вторая строка будет содержать одно целое число m - количество задач, которые надо решить при оптимальном выводе
            Третья строка будет содержать n разделённых пробелом целых чисел - номера этих задачь в порядке их решения!
            Задачи пронумерованы начиная с единицы, в порядке поступления во входном файле
*/
